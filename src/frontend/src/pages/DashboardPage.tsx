...
import { Database, Shield, Activity, AlertTriangle, TrendingUp, FileText } from 'lucide-react';
import { useInternetIdentity } from '../hooks/useInternetIdentity';
import { useGetCallerBusinessProfile, useGetBusinessFiles, useIsCallerAdmin, useGetCustomerBillingStatus } from '../hooks/useQueries';
...
export default function DashboardPage() {
  ...
  const totalStorageBytes = files.reduce((sum, file) => sum + Number(file.sizeBytes), 0);
  const totalStorageGB = (totalStorageBytes / (1024 ** 3)).toFixed(2);
  const includedStorageGB = Number(profile.storageEstimateGb);
  const storageUsagePercent = (parseFloat(totalStorageGB) / includedStorageGB) * 100;
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        {billingStatus && billingStatus !== BillingStatus.active ? '1' : '0'}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {billingStatus && billingStatus !== BillingStatus.active ? 'Billing issue' : 'No active alerts'}
                      </p>
                    </CardContent>
                  </Card>
                </>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{profile.subscriptionTier} Plan</p>
